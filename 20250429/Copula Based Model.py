...
Lognormal_Mu_Parameters = underwriting_df['Lognormal_Mu_Parameter'].tolist()
Lognormal_Sigma_Parameters = underwriting_df['Lognormal_Sigma_Parameter'].tolist()
reserve_params_frequency = reserve_df['Frequency_Parameter'].tolist()
reserve_params_severity_mu = reserve_df['Severity_Mu_Parameter'].tolist()
reserve_params_severity_sigma = reserve_df['Severity_Sigma_Parameter'].tolist()
...
CAT_losses_by_lob = ProteusVariable(
    "class",
    values={
        lob: distributions.LogNormal(mu=Lognormal_Mu_Parameters[idx],sigma=Lognormal_Sigma_Parameters[idx]).generate()
        for idx, lob in enumerate(lobs)
    },
)
...
copulas.StudentsTCopula(correlation_matrix, len(lobs)).apply(aggregate_reserve_risk_by_lob)
...
credit_risk = primary_exposure * (high_quality_pd * lgd * ead + medium_quality_pd * lgd * ead + low_quality_pd * lgd * ead)
...
copulas.GumbelCopula(Risk_Aggregation_Theta, 7).apply([total_net_losses,reserve_risk,interest_rate_changes,equity_price_changes,currency_exchange_rate_changes,commodity_price_changes,operational_risk])
combined_risk_values = total_net_losses.values + reserve_risk.values + expense_risk.values + interest_rate_changes.values + equity_price_changes.values + currency_exchange_rate_changes.values + commodity_price_changes.values + credit_risk.values + operational_risk.values 
combined_risk = StochasticScalar(values=combined_risk_values)


#Show CDFs
total_net_losses.show_cdf()
reserve_risk.show_cdf()
expense_risk.show_cdf()
interest_rate_changes.show_cdf()
equity_price_changes.show_cdf()
currency_exchange_rate_changes.show_cdf()
commodity_price_changes.show_cdf()
...
fig.add_trace(go.Scatter(x=np.array(total_net_losses), y=np.array(interest_rate_changes), mode='markers', name='Interest Rate Changes'))
fig.add_trace(go.Scatter(x=np.array(total_net_losses), y=np.array(equity_price_changes), mode='markers', name='Equity Price Changes'))
fig.add_trace(go.Scatter(x=np.array(total_net_losses), y=np.array(currency_exchange_rate_changes), mode='markers', name='Currency Exchange Rate Changes'))
fig.add_trace(go.Scatter(x=np.array(total_net_losses), y=np.array(commodity_price_changes), mode='markers', name='Commodity Price Changes'))
...
losses_data = pd.DataFrame({
    ...
    "Interest Rate Changes": interest_rate_changes.values,
    "Equity Price Changes": equity_price_changes.values,
    "Currency Exchange Rate Changes": currency_exchange_rate_changes.values,
    "Commodity Price Changes": commodity_price_changes.values,
...
contribution_data_VAR = pd.DataFrame({
    ...
    "Interest Rate Changes": [interest_rate_changes.percentile(Selected_Percentile)/combined_risk.percentile(Selected_Percentile)],
    "Equity Price Changes": [equity_price_changes.percentile(Selected_Percentile)/combined_risk.percentile(Selected_Percentile)],
    "Currency Exchange Rate Changes": [currency_exchange_rate_changes.percentile(Selected_Percentile)/combined_risk.percentile(Selected_Percentile)],
    "Commodity Price Changes": [commodity_price_changes.percentile(Selected_Percentile)/combined_risk.percentile(Selected_Percentile)],
...
contribution_data_tVAR = pd.DataFrame({
    ...
    "Interest Rate Changes": [interest_rate_changes.tvar(Selected_Percentile)/combined_risk.tvar(Selected_Percentile)],
    "Equity Price Changes": [equity_price_changes.tvar(Selected_Percentile)/combined_risk.tvar(Selected_Percentile)],
    "Currency Exchange Rate Changes": [currency_exchange_rate_changes.tvar(Selected_Percentile)/combined_risk.tvar(Selected_Percentile)],
    "Commodity Price Changes": [commodity_price_changes.tvar(Selected_Percentile)/combined_risk.tvar(Selected_Percentile)],
...
discounted_losses = ProteusVariable(
    dim_name="class",
    values={
        name: (total_net_losses_by_lob[name].values + aggregate_reserve_risk_by_lob[name].values) * discount_factor_df[name]
        for name in lobs
    },
)
...
print(f"Average Discounting Impact: {np.mean(Discounting_Impact)}")# IFOA_WG_Capital_Models
