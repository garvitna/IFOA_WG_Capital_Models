...
market_corr_df = pd.read_excel(file_path, sheet_name='Market_Correlation', index_col=0)
...
Individual_XOL_Retention = reinsurance_df['Individual_Retention'].tolist()
Individual_XOL_Limit = reinsurance_df['Individual_Limit'].tolist()
correlation_matrix = corr_df.values
market_correlation_matrix = market_corr_df.values
...
Net_attritional_losses_by_lob = ProteusVariable(
    "class",
    values={
        lob: attritional_losses_by_lob[lob] * (1 - Quota_Share[idx])
        for idx,lob in enumerate(lobs)
    },
)
...
Net_QS_aggregate_large_losses_by_class = ProteusVariable(
    "class",
    values={
        lob: aggregate_large_losses_by_class[lob] * (1 - Quota_Share[idx])
        for idx,lob in enumerate(lobs)
    },
)
...
Net_QS_CAT_losses_by_lob = ProteusVariable(
    "class",
    values={
        lob: CAT_losses_by_lob[lob] * (1 - Quota_Share[idx])
        for idx,lob in enumerate(lobs)
    },
)
...
total_gross_losses_by_lob = aggregate_large_losses_by_class + attritional_losses_by_lob + CAT_losses_by_lob
...
underwriting_risk = StochasticScalar(
    values=total_net_losses.values  
)
...
total_market_risks = [interest_rate_changes, equity_price_changes, currency_exchange_rate_changes, commodity_price_changes, credit_spread]

copulas.StudentsTCopula(market_corr_df, 5).apply(total_market_risks)

market_risks_values = np.maximum(
    interest_rate_changes.values + equity_price_changes.values + currency_exchange_rate_changes.values + commodity_price_changes.values + credit_spread.values, 0
)


Market_risk = StochasticScalar(
    values=market_risks_values*adjusted_assets
)
...
copulas.GumbelCopula(Risk_Aggregation_Theta, 4).apply([underwriting_risk,reserve_risk,Market_risk,operational_risk])
...
contribution_data_VAR = pd.DataFrame({
    "Underwriting Risk": [underwriting_risk.percentile(Selected_Percentile)/combined_risk.percentile(Selected_Percentile)],
...
contribution_data_tVAR = pd.DataFrame({
    "Underwriting Risk": [underwriting_risk.tvar(Selected_Percentile)/combined_risk.tvar(Selected_Percentile)],
...
with pd.ExcelWriter('Copula_Based_Model_Output.xlsx') as writer:
