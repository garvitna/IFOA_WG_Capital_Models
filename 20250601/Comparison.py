...
combined_corr = pd.concat([copula_corr, driver_corr, hybrid_corr], axis=1, keys=['Copula-Based', 'Driver-Based', 'Hybrid-Based'])
...
combined_jep = pd.concat([copula_jep, driver_jep, hybrid_jep], axis=1, keys=['Copula-Based', 'Driver-Based', 'Hybrid-Based'])

# Load Discounting Impact Sheets
copula_discounting = pd.read_excel(copula_file, sheet_name='Discounting_Impact')
driver_discounting = pd.read_excel(driver_file, sheet_name='Discounting_Impact')
hybrid_discounting = pd.read_excel(hybrid_file, sheet_name='Discounting_Impact')

discounting_comparison = pd.DataFrame({
    'Model': ['Copula-Based', 'Driver-Based', 'Hybrid-Based'],
    'Average Discounting Impact': [
        copula_discounting['Average_Discounting_Impact'].iloc[0],
        driver_discounting['Average_Discounting_Impact'].iloc[0],
        hybrid_discounting['Average_Discounting_Impact'].iloc[0]
    ]
})
...
# Contrubution to VaR and TVaR Data
def contribution_stacked(sheet_name, title):
    dfs = [
        pd.read_excel(copula_file, sheet_name=sheet_name),
        pd.read_excel(driver_file, sheet_name=sheet_name),
        pd.read_excel(hybrid_file, sheet_name=sheet_name)
...
contribution_var = contribution_stacked('Contribution_data_VAR', 'Contribution to VaR')
contribution_tvar = contribution_stacked('Contribution_data_tVAR', 'Contribution to TVaR')
...
with pd.ExcelWriter('comparison_Output.xlsx') as writer:
    ...
    contribution_var.to_excel(writer, sheet_name='Contribution_VaR', index=True)
    contribution_tvar.to_excel(writer, sheet_name='Contribution_TVaR', index=True)
    combined_jep.to_excel(writer, sheet_name='Combined_JEP', index=True)
    discounting_comparison.to_excel(writer, sheet_name='Discounting_Impact', index=False)
