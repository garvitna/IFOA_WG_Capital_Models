...
for idx, lob in enumerate(lobs):
    reserve_risk_by_lob = reserve_risk_by_lob * Currency_Exchange_Class_Impact[idx] * Economic_Cycle_Class_Impact[idx] * (1+(underwriting_cycle * Underwriting_Cycle_class_impact[idx]))
...
combined_risk_values = total_inflated_losses.values + reserve_risk.values + expense_risk.values + Market_risk.values + credit_risk.values + operational_risk.values
combined_risk = StochasticScalar(values=combined_risk_values)

#Show CDFs
total_inflated_losses.show_cdf(title="Underwriting Risk")
...
fig.add_trace(go.Scatter(x=np.array(total_inflated_losses), y=np.array(Market_risk), mode='markers', name='Market Risk'))
...
total_risks = pd.DataFrame({
    ...
    "Underwriting Risk": total_inflated_losses.values,
    "Reserve Risk": reserve_risk.values,
    "Expense Risk": expense_risk.values,
    "Market Risk": Market_risk.values,
    "Credit Risk": credit_risk.values,
